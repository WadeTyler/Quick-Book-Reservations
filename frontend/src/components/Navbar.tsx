...
import {fetchUser, logout} from "@/features/auth/lib/auth.service";
        ...
        {!isLoadingAuthUser && authUser && !isLoggingOut && (
          <div className="relative">
            <div
              ...
              className="w-8 h-8 bg-background-secondary rounded-full flex items-center justify-center group hover:bg-accent cursor-pointer duration-200 text-foreground">
              <RiUserFill className="group-hover:text-white duration-200"/>
            </div>

            {isShowingUserMenu && (
              <ClickAwayListener onClickAway={() => setIsShowingUserMenu(false)}>
                <div onClick={() => setIsShowingUserMenu(false)}
                     className="absolute right-0 top-full mt-2 w-fit bg-background-secondary rounded-md shadow-md flex flex-col text-accent p-2 gap-1 items-center justify-center">

                  <Link href="/account"
                        className="w-full text-center hover:bg-accent hover:text-white duration-200 rounded-md px-2 py-1">Account</Link>

                  <button onClick={() => handleLogout()}
                          className="w-full text-center hover:bg-accent hover:text-white duration-200 rounded-md px-2 py-1 cursor-pointer">
                    Logout
                  </button>
