...
import {deleteBusiness, fetchManagedBusinessById} from "@/features/business/lib/manage-business.service";
...
const DeleteBusinessForm = () => {
  ...
  const {mutate: handleDelete, isPending: isDeleting, error: deleteError} = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({queryKey: ['managedBusiness']});
      await queryClient.invalidateQueries({queryKey: ['managedBusinesses']});
